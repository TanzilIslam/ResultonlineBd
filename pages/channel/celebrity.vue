      ...
      <b-col cols="12" sm="12" md="9" lg="9" xl="9">
        ...
        <div class="channel-cover">
          <b-card
            overlay
            img-height="80"
            img-src="~/assets/user/icons/channel-cover-celebrity.jpg"
            text-variant="white"
          >
            <b-card-text text-tag="h2" class="channel-cover-title">
              Celebrity & Movie</b-card-text
            >
          </b-card>
        </div>
...
export default {
  ...
  async fetch() {
    ...
    await self.$axios
      .$get(process.env.baseUrl + "/s/all/celebrity")
      .then(function(posts) {
    ...
    // Main Tag List Fetch
    await this.$axios
      .$get(process.env.channelMainTag + `celebrity`)
      .then(function(posts) {
    ...
    // Sub Tag List Fetch
    await this.$axios
      .$get(process.env.baseUrl + "/Tag_creator?search=Celebrity")
      .then(function(posts) {
    ...
    await this.$axios
      .$get(process.env.channelUrl + `Celebrity`)
      .then(posts =>
  ...
  methods: {
    ...
    async showMainTagPosts(item) {
      ...
      await this.$axios
        ...
        .then(function(posts) {
          self.$store.dispatch(
            "celebrity/FetchCelebrityArticles",
            posts.results
          );
