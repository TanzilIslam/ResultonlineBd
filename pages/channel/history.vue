<template>
  <div class="history-post">
    <!-- Cover Start -->
    <ChannelCover ChannelCoverTitle="History" />
    <!-- Cover End -->
...
export default {
      ...
      meta: [
        {
          ...
          name: "description",
          content:
            "Here you can find all the latest information about HistoryArticles "
        }
      ]
    };
  },
  async fetch({ store, error }) {
    try {
      await store.dispatch("FetchHistoryArticles");
    } catch (e) {
      error({
        statusCode: 503,
  ...
  computed: mapState({
    HistoryArticles: state => state.HistoryArticles
  }),
  data() {
    return {
      showLatestDiv: true,
        ...
        let moreData = await this.$axios
          ...
          .then(item =>
            item.results.forEach(element => {
              this.$store.dispatch("FetchMoreHistoryArticles", element);
            })
          );
